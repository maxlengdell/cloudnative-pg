...
var _ = Describe("Namespaced Deployment", Label(tests.LabelNoOpenshift, tests.LabelDisruptive,
	...
	BeforeAll(func() {
		if os.Getenv("NAMESPACED") != "true" {
			Skip("Skipping namespaced deployment test because namespaced deployment is not enabled")
		}
		if IsOpenshift() {
	...
	BeforeEach(func() {
		...
		By("deploying the namespaced operator", func() {
			_, err := os.Stat(namespacedOperatorManifest)
			Expect(err).NotTo(HaveOccurred(), "namespaced manifest should exist")

			_, stderr, err := run.Run(
				fmt.Sprintf("kubectl apply --server-side --force-conflicts -f %v", namespacedOperatorManifest))
